# AList Player

<div align="center">
  <img src="assets/icon/logo.png" alt="AList Player Logo" width="128" height="128">

  <h3>基于 AList 的跨平台视频播放器</h3>

  <p>
    <strong>支持进度保存、历史浏览记录的专业媒体播放应用</strong>
  </p>

  <p>
    <a href="#功能特性">功能特性</a> •
    <a href="#安装说明">安装说明</a> •
    <a href="#使用指南">使用指南</a> •
    <a href="#技术架构">技术架构</a> •
    <a href="#开发指南">开发指南</a>
  </p>
</div>

---

## 📖 项目简介

AList Player 是一款基于 [AList](https://alist.nn.ci/) 的跨平台视频播放器，使用 Flutter 开发，支持 Android、iOS、Windows、macOS、Linux 和 Web 平台。该应用专为媒体文件管理和播放而设计，提供了丰富的功能和优秀的用户体验。

### 🎯 核心优势

- **🔗 AList 集成**: 完美集成 AList 文件管理系统，支持多种存储后端
- **📱 跨平台支持**: 一套代码，支持所有主流平台
- **🎬 专业播放**: 基于 media_kit 的高性能视频播放引擎
- **💾 智能记忆**: 自动保存播放进度和历史记录
- **🎨 现代界面**: Material Design 3 设计语言，界面美观易用
- **⚡ 高性能**: 硬件加速解码，支持 4K/8K 视频播放

## ✨ 功能特性

### 🎥 媒体播放
- **多格式支持**: 支持几乎所有主流视频和音频格式
- **硬件加速**: GPU 加速解码，流畅播放高清视频
- **播放控制**: 完整的播放控制功能（播放/暂停/快进/快退/音量调节）
- **字幕支持**: 支持内嵌字幕和外挂字幕文件
- **播放列表**: 支持播放列表管理和连续播放
- **倍速播放**: 支持 0.25x - 4x 倍速播放

### 📚 文件管理
- **AList 集成**: 直接访问 AList 管理的文件系统
- **多存储支持**: 支持本地存储、云存储等多种存储后端
- **文件浏览**: 直观的文件浏览界面，支持文件夹导航
- **搜索功能**: 快速搜索文件和文件夹
- **收藏夹**: 收藏常用文件夹，快速访问
- **批量操作**: 支持批量选择和操作文件

### 💾 数据管理
- **播放进度**: 自动保存和恢复播放进度
- **历史记录**: 完整的播放历史记录管理
- **数据库支持**: 使用 PostgreSQL 存储用户数据
- **数据同步**: 支持多设备间数据同步
- **配置管理**: 灵活的 API 配置和数据库配置管理

### 🔧 高级功能
- **下载管理**: 支持文件下载和离线播放
- **配置服务器**: 内置配置服务器，便于设备间配置同步
- **日志系统**: 完整的日志记录和错误追踪
- **主题定制**: 支持主题定制和界面个性化
- **快捷键**: 丰富的键盘快捷键支持（桌面平台）

## 🚀 安装说明

### 系统要求

| 平台 | 最低版本 | 推荐版本 |
|------|----------|----------|
| Android | 5.0 (API 21) | 8.0+ |
| iOS | 9.0 | 12.0+ |
| Windows | Windows 7 | Windows 10+ |
| macOS | 10.9 | 11.0+ |
| Linux | 任何现代发行版 | Ubuntu 20.04+ |
| Web | 任何现代浏览器 | Chrome 90+ |

### 预编译版本

1. 访问 [Releases](../../releases) 页面
2. 下载适合您平台的安装包
3. 按照平台说明进行安装

### 从源码构建

#### 环境准备

```bash
# 安装 Flutter SDK (>= 3.4.1)
git clone https://github.com/flutter/flutter.git
export PATH="$PATH:`pwd`/flutter/bin"

# 验证环境
flutter doctor
```

#### 构建步骤

```bash
# 克隆项目
git clone <repository-url>
cd webdav_video

# 安装依赖
flutter pub get

# 生成应用图标
flutter pub run flutter_launcher_icons

# 构建应用
flutter build <platform>
```

#### 平台特定构建

```bash
# Android
flutter build apk --release
flutter build appbundle --release

# iOS
flutter build ios --release

# Windows
flutter build windows --release

# macOS
flutter build macos --release

# Linux
flutter build linux --release

# Web
flutter build web --release
```

## 📱 使用指南

### 首次配置

1. **启动应用**: 首次启动会显示登录界面
2. **API 配置**: 点击"API 设置"配置 AList 服务器地址
3. **数据库配置**: 点击"数据库设置"配置 PostgreSQL 连接（可选）
4. **登录**: 输入 AList 用户名和密码登录

### 基本操作

#### 文件浏览
- 点击文件夹进入子目录
- 点击视频文件开始播放
- 使用面包屑导航快速返回上级目录
- 点击搜索图标搜索文件

#### 视频播放
- 单击屏幕显示/隐藏控制栏
- 双击屏幕切换全屏模式
- 拖拽进度条调整播放位置
- 使用音量和亮度手势控制

#### 播放列表
- 播放视频时自动生成播放列表
- 点击播放列表中的项目切换视频
- 支持上一个/下一个快速切换
- 支持播放模式设置（顺序/循环/随机）

### 高级功能

#### 收藏夹管理
1. 在文件浏览界面点击星形图标
2. 添加当前目录到收藏夹
3. 在主界面快速访问收藏的目录

#### 下载管理
1. 长按文件选择下载
2. 在下载管理界面查看下载进度
3. 下载完成后可离线播放

#### 历史记录
- 自动记录播放历史
- 在历史记录界面查看和管理
- 支持清除历史记录

## 🏗️ 技术架构

### 核心技术栈

| 技术 | 版本 | 用途 |
|------|------|------|
| Flutter | 3.4.1+ | 跨平台 UI 框架 |
| Dart | 3.0+ | 编程语言 |
| media_kit | 1.2.0+ | 媒体播放引擎 |
| PostgreSQL | - | 数据存储 |
| Dio | 5.7.0+ | HTTP 客户端 |

### 项目结构

```
lib/
├── apis/           # API 接口层
├── constants/      # 常量定义
├── models/         # 数据模型
├── services/       # 业务服务层
├── utils/          # 工具类
├── views/          # 界面层
├── widgets/        # 自定义组件
└── main.dart       # 应用入口

# 🛠️ 本地 Go 数据库桥

部分场景需要通过本地服务代理 MySQL / PostgreSQL / Oracle 等传统数据库。仓库自带 `go/go_bridge` 工程，可按如下步骤启用：

1. 根据 `go/go_bridge/config.yaml` 调整数据库驱动与 DSN；如需自定义路径，可设置环境变量 `GO_BRIDGE_CONFIG=/path/to/config.yaml`。
2. 在本机安装 Go 1.21+，执行：

```bash
cd go/go_bridge
go run .
```

3. 启动成功后，访问 `http://127.0.0.1:7788/health` 校验；Flutter 端在数据库设置中选择“本地 Go 服务”并填入地址/Token，即可走该桥接层。

详细接口契约及部署建议见 `go/go_bridge/README.md`。

# 🚀 生产构建脚本

项目提供统一脚本 `scripts/build_release.sh`，可一键构建不同平台的发行包，亦可编译 Go 数据库桥：

```bash
./scripts/build_release.sh android-apk      # 生成 Android APK
./scripts/build_release.sh ios             # 构建 iOS Release
./scripts/build_release.sh macos           # 构建 macOS 桌面端
./scripts/build_release.sh windows         # 构建 Windows 桌面端
./scripts/build_release.sh linux           # 构建 Linux 桌面端
./scripts/build_release.sh web             # 构建 Web 发行版
cd go/go_bridge && ./build_release.sh      # 编译 Go 本地数据库桥（支持 GOOS/GOARCH）
```

在运行脚本前请确保已执行 `flutter pub get`，并准备好相应平台的签名/证书环境。

assets/
├── config/         # 配置文件
├── fonts/          # 字体文件
├── icon/           # 应用图标
└── images/         # 图片资源
```

### 核心组件

#### 媒体播放引擎
- 基于 `media_kit` 的跨平台播放引擎
- 支持硬件加速和软件解码
- 完整的播放状态管理和事件处理

#### 数据管理层
- PostgreSQL 数据库存储用户数据
- SharedPreferences 存储应用配置
- 完整的数据迁移和版本管理

#### 网络层
- 基于 Dio 的 HTTP 客户端
- 支持请求拦截和响应处理
- 完整的错误处理和重试机制

#### 配置管理
- 灵活的 API 配置预设系统
- 数据库连接配置管理
- 应用设置和用户偏好管理

## 🛠️ 开发指南

### 开发环境设置

1. **安装 Flutter SDK**
   ```bash
   # 下载并安装 Flutter SDK
   # 配置环境变量
   export PATH="$PATH:/path/to/flutter/bin"
   ```

2. **IDE 配置**
   - 推荐使用 VS Code 或 Android Studio
   - 安装 Flutter 和 Dart 插件
   - 配置代码格式化和 lint 规则

3. **依赖安装**
   ```bash
   flutter pub get
   ```

### 代码规范

#### 命名规范
- 文件名：使用下划线分隔的小写字母
- 类名：使用 PascalCase
- 变量和方法：使用 camelCase
- 常量：使用 UPPER_SNAKE_CASE

#### 代码组织
- 按功能模块组织代码
- 使用 barrel exports 简化导入
- 保持文件大小适中（< 500 行）

#### 注释规范
```dart
/// 播放器控制器
///
/// 负责管理视频播放状态和用户交互
class PlayerController {
  /// 播放指定的媒体文件
  ///
  /// [media] 要播放的媒体对象
  /// [autoPlay] 是否自动开始播放
  Future<void> play(Media media, {bool autoPlay = true}) async {
    // 实现代码
  }
}
```

### 测试指南

#### 单元测试
```bash
# 运行所有测试
flutter test

# 运行特定测试文件
flutter test test/unit/player_test.dart

# 生成测试覆盖率报告
flutter test --coverage
```

#### 集成测试
```bash
# 运行集成测试
flutter test integration_test/
```

#### 性能测试
```bash
# 性能分析
flutter run --profile
```

### 构建和发布

#### 版本管理
1. 更新 `pubspec.yaml` 中的版本号
2. 更新 `CHANGELOG.md`
3. 创建 Git 标签

#### 发布流程
```bash
# 1. 代码检查
flutter analyze
flutter test

# 2. 构建发布版本
flutter build apk --release
flutter build appbundle --release

# 3. 签名和发布
# 按照各平台要求进行签名和发布
```

### 贡献指南

1. **Fork 项目**
2. **创建功能分支**
   ```bash
   git checkout -b feature/new-feature
   ```
3. **提交更改**
   ```bash
   git commit -m "Add new feature"
   ```
4. **推送分支**
   ```bash
   git push origin feature/new-feature
   ```
5. **创建 Pull Request**

### 调试技巧

#### 日志调试
```dart
import 'package:alist_player/utils/logger.dart';

// 记录调试信息
await AppLogger().debug('Player', 'Starting playback');

// 记录错误信息
await AppLogger().error('Player', 'Playback failed: $error');
```

#### 性能调试
```bash
# 启用性能监控
flutter run --profile

# 使用 Flutter Inspector
flutter inspector
```

## 📄 许可证

本项目采用 [MIT 许可证](LICENSE)。

## 🤝 贡献

欢迎贡献代码！请查看 [贡献指南](CONTRIBUTING.md) 了解详细信息。

## 📞 支持

- 🐛 [报告问题](../../issues)
- 💡 [功能建议](../../issues)
- 📧 [联系我们](mailto:support@example.com)
- 📖 [文档](../../wiki)

## 🙏 致谢

- [AList](https://alist.nn.ci/) - 优秀的文件管理系统
- [media_kit](https://github.com/media-kit/media-kit) - 强大的媒体播放引擎
- [Flutter](https://flutter.dev/) - 跨平台 UI 框架
- 所有贡献者和用户的支持

---

<div align="center">
  <p>如果这个项目对您有帮助，请给我们一个 ⭐️</p>
  <p>Made with ❤️ by AList Player Team</p>
</div>
