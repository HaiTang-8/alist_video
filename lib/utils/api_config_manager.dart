...
class ApiConfigManager {
  ...
  Future<bool> setCustomApiMode(bool isCustom) async {
    try {
      ...
      print('设置自定义API模式失败: $e');
      return false;
    }
  }

  /// 从当前设置创建新的配置预设
  Future<ApiConfigPreset> createPresetFromCurrentSettings(String name, {String? description}) async {
    final prefs = await SharedPreferences.getInstance();
  ...
  Future<void> _applyPresetToCurrentSettings(ApiConfigPreset preset) async {
    try {
      ...
      await Future.wait([
        ...
        prefs.setString(AppConstants.baseDownloadUrlKey, preset.baseDownloadUrl),
      ]);
